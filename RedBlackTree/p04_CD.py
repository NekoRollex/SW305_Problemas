...
class ArbolRB:
    ...
    def imprimir_colores(self, nodo=None):
        if nodo is None:
            nodo = self.root
        padre_valor = nodo.parent.value if nodo.parent else "Ninguno"
        print(f"Valor: {nodo.value}, Color: {nodo.color}, Padre: {padre_valor}")
        if nodo.left:
            self.imprimir_colores(nodo.left)
        if nodo.right:
            self.imprimir_colores(nodo.right)


arbol = ArbolRB()
valores = [20, 15, 25, 10, 5, 1]
for valor in valores:
    arbol.insert(valor)
print("Recorrido en orden del árbol rojo-negro:")
arbol._inorder_traversal(arbol.root)
print()
arbol.imprimir_colores()
print()

valor_eliminar = int(input("Ingrese el valor que desea eliminar: "))
arbol.delete(valor_eliminar)

print(f"Recorrido en orden del árbol rojo-negro después de eliminar {valor_eliminar}")
arbol._inorder_traversal(arbol.root)
print()
arbol.imprimir_colores()