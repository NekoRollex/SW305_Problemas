...
class AVLTree:
    ...
    def eliminar(self, raiz, valor):
        ...
        if valor < raiz.valor:
            ...
            # Nodo con dos hijos, encontrar el sucesor (el más pequeño en el subárbol derecho)
    ...
    def contar_nodos(self, raiz):
        if not raiz:
            return 0
        # Cuenta el nodo actual y recursivamente en los subárboles izquierdo y derecho
        return 1 + self.contar_nodos(raiz.izquierda) + self.contar_nodos(raiz.derecha)
...
num = arbol.contar_nodos(arbol.raiz)
print(f"El número total de nodos en el árbol es: {num}")
